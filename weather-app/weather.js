...
function getWeather(){
    ...
    if(city != ''){
        ...
        $.ajax({
            url:'http://api.openweathermap.org/data/2.5/weather?q=' + city + "&units=metric" + "&APPID=d4e068b96c2cbcf6d89f729412fec0e2",
            type:"GET",
            dataType:"jsonp",
...
function showResults(data){
    return '<h2 style="font-weight:bold;font-size:30px; padding-top:20px;" class="text-center">Current Weather for  '+data.name+', '+data.sys.country+'</h2>'+
        ...
        "<h3 style='padding-left:40px;'><strong>Temperture</strong>: "+data.main.temp+" &deg;C</h3>"+
        ...
        "<h3 style='padding-left:40px;'><strong>Min Temperture</strong>: "+data.main.temp_min+" &deg;C</h3>"+
        "<h3 style='padding-left:40px;'><strong>Max Temperture</strong>: "+data.main.temp_max+" &deg;C</h3>"+
        "<h3 style='padding-left:40px;'><strong>Wind Speed</strong>: "+data.wind.speed+"m/s</h3>"+
        "<h3 style='padding-left:40px; padding-bottom:30px;'><strong>Wind Directionv: "+data.wind.deg+" &deg;</h3>"
}